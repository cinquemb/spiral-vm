...
void SpiralVM::ramp_omega_ang(double start, double end, double duration_seconds) {
...
cx_mat SpiralVM::mat_vec_mult_cl10(const sp_cx_mat& H, const cx_mat& phi) {
    return H * phi;
}

double SpiralVM::inner_product_cl10(const cx_mat& phi1, const cx_mat& phi2) {
    cx_double prod = as_scalar(phi1.t() * phi2);
    return real(prod);
}

double SpiralVM::compute_zz_energy(const cx_mat& phi, double J, double omega_ang, double period, bool is_ang) {
    double energy = 0.0;
...
cx_mat SpiralVM::compute_zz_energy_vector(const cx_mat& phi, double J, double omega_ang, double period, bool is_ang) {
...
void SpiralVM::compute_nonzero_indices_spiral_twist(double J, double ht, int rows, int cols, int D, double omega_ang, umat& locations, cx_vec& values, uint& nz) {
    int N = rows * cols;
...
sp_cx_mat SpiralVM::hamiltonian_cl10_90_spiral_twist(double J, double ht, double omega_ang) {
...
double SpiralVM::compute_avg_stabilizer(const cx_mat& phi) {
    double stab_sum = 0.0;
    int count = 0;
